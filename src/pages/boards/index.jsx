...
/**
  /libs/hooks/usePaginator, /components/common/Paginator 참고해서 리팩토링 해주세요.
...
export default function QNAList() {
  ...
  const onClickNextPage = () => {
    if (startPage + 10 <= lastPage) {
      // startPage 값은 마지막 페이지 값보다 무조건 작거나 같아야 한다.
  ...
  return (
    <main className="w-1200px m-100px">
      ...
      {data.map((el) => (
        <section
          className="flex flex-row h-52px leading-52px border-b-1 border-b-gray-400 border-b-solid hover:text-blue-400"
          key={uuidv4()}
        >
          <div className="w-1/12 text-center">{el?.id}</div>
          <div
            className="w-7/12 text-center cursor-pointer"
